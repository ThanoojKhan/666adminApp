...
const Table = () => {
  ...
  const fetchEnquiries = async (page) => {
    try {
      ...
      const snapshot = await getDocs(query(enquiriesRef, ...queryParams));
      ...
      const fetchedEnquiries = docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
        createdAt: doc.data().createdAt.toDate().toLocaleString()
      }));

      const updatedEnquiries = page === 1 ? fetchedEnquiries : [...enquiries, ...fetchedEnquiries];
